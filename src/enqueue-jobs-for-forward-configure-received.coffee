...
class EnqueueJobsForForwardConfigureReceived
  constructor: (options) ->
  ...
  _enqueueForReceived: ({forwardedRoutes, rawData, route, uuid}, callback) =>
    ...
    @_resolveUuids {from, to, uuid}, (error, {from, to, uuid}) =>
      ...
      @datastore.findOne {uuid}, projection, (error, device) =>
        ...
        forwarders = _.get device.meshblu?.forwarders, 'configure.received'
  ...
  _createRequests: (forwarders, {forwardedRoutes, rawData, route, uuid}, callback)=>
    ...
    async.eachSeries forwarders, createRequest, callback
...
module.exports = EnqueueJobsForForwardConfigureReceived
